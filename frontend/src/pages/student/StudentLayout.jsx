...
const navItems = [
  ...
  { to: '/dashboard/discover', label: 'Discover Events', icon: Search },
  { to: '/dashboard/schedule', label: 'Schedule', icon: Calendar },
  { to: '/dashboard/my-events', label: 'My Events', icon: History },
]

const titleByPath = {
  '/dashboard': 'DASHBOARD',
  '/dashboard/discover': 'DISCOVER EVENTS',
    ...
    <div className="min-h-screen bg-[#f3f4f6]">
      ...
      >
        <div className="flex items-start justify-between gap-2">
          <div className={collapsed ? 'hidden' : 'block'}>
            ...
            <p className="text-xs uppercase tracking-widest text-slate-400 mt-1">Student Portal</p>
          </div>
          <button
            type="button"
      ...
      <div className={`${collapsed ? 'ml-32' : 'ml-[312px]'} mr-4 py-4 transition-all duration-200`}>
        <header className="bg-white rounded-2xl shadow-sm border border-slate-100 px-5 py-4 flex items-center justify-between gap-4">
          <h2 className="text-sm sm:text-base font-extrabold tracking-widest text-slate-600">{pageTitle}</h2>
